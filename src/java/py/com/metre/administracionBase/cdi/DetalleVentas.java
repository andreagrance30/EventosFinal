...
public class DetalleVentas implements Serializable {
    ...
    Logger logger = Logger.getLogger(DetalleVentas.class);
    @PersistenceContext
    ...
    private List<ItemVendido> listaDetalle;
    ...
    public void generarPDF(){
        ...
        imprimirreporte.setNombreReporte("DetalleVentas");
    ...
    private String getIdPedidoFinal(){
     ...
     private String consultaDetalle(){
        ...
        "FROM item i, detalle_factura df, detalle_pedido dp, factura f " +
        "WHERE f.id_factura = df.id_factura " +
        "AND df.id_detalle_pedido = dp.id_detalle " +
        "AND dp.id_item = i.id_item " +
         ...
         */
        if(idPedidoInicial.isEmpty()){
            query += " AND dp.cancelado = TRUE ";
        }

        query += " GROUP by i.nombre " +
        "ORDER BY CANTIDAD desc, ITEM ASC";
